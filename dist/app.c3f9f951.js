...
},{"./..\\images\\back.png":[["back.177f3c7d.png","images/back.png"],"images/back.png"],"_css_loader":"../node_modules/parcel-bundler/src/builtins/css-loader.js"}],"js/modules/budgetController.js":[function(require,module,exports) {
"use strict";
...
function _default() {
  ...
  var Expense = /*#__PURE__*/function () {
    ...
    _createClass(Expense, [{
      key: "getIndividualPercentage",
      value: function getIndividualPercentage() {
        if (data.budget > 0) {
  ...
  return {
    addItem: function addItem(obj, type) {
      var ID, newItem; // create an id
      ...
      data.totals[type] += newItem.value; // update totals

      this.calculateBudget(); // calculate percentage
    ...
    deleteItem: function deleteItem(elem) {
      ...
      var elemId = parseInt(elem.split('-')[1]); // find item's index

      var allIds = data.allItems[type].map(function (el) {
        return el.id;
      });
      var itemIndex = allIds.indexOf(elemId);
      if (itemIndex === -1) return; // subtract item from totals

      var item = data.allItems[type][itemIndex];
      data.totals[type] = data.totals[type] - item.value; // remove item from data structure

      data.allItems[type].splice(itemIndex, 1); // update budget in data structure
...
},{}],"js/modules/uiController.js":[function(require,module,exports) {
"use strict";
...
function _default() {
  ...
  return {
    ...
    addElement: function addElement(obj, type) {
      var markup, element;

      if (type === 'inc') {
        element = document.querySelector(DOMstrings.incomeList);
        markup = "<div class=\"item clearfix\" id=\"inc-".concat(obj.id, "\"><div class=\"item__description\">").concat(obj.description, "</div><div class=\"right clearfix\"><div class=\"item__value\">").concat(formatNumber(obj.value, type), "</div><div class=\"item__delete\"><button class=\"item__delete--btn\"><i class=\"ion-ios-close-outline\"></i></button></div></div></div>");
      } else if (type === 'exp') {
        element = document.querySelector(DOMstrings.expensesList);
        markup = "<div class=\"item clearfix\" id=\"exp-".concat(obj.id, "\"><div class=\"item__description\">").concat(obj.description, "</div><div class=\"right clearfix\"><div class=\"item__value\">").concat(formatNumber(obj.value, type), "</div><div class=\"item__percentage\">21%</div><div class=\"item__delete\"><button class=\"item__delete--btn\"><i class=\"ion-ios-close-outline\"></i></button></div></div></div>");
      }

      element.insertAdjacentHTML('beforeend', markup);
    },
    displayPercentages: function displayPercentages(obj, percentages) {
      var nodeList = document.querySelectorAll(DOMstrings.itemPercentageLabel);
    ...
    displayMonth: function displayMonth() {
      var dateNow = new Date(Date.now()).toLocaleString('en-us', {
        month: 'long',
        year: 'numeric'
      });
...
var _budgetController = _interopRequireDefault(require("./modules/budgetController"));

var _uiController = _interopRequireDefault(require("./modules/uiController"));
...
var controller = function (budgetCtrl, UIctrl) {
  function setupEventListeners() {
    ...
    document.querySelector(DOMstr.addButton).addEventListener('click', ctrlAddItem); // add item
    ...
    document.querySelector(DOMstr.container).addEventListener('click', function (e) {
      var element = e.target.parentElement.parentElement.parentElement.parentElement;
  ...
  function ctrlAddItem() {
    // get input values
    var inputValues = UIctrl.getInputValues();
    if (inputValues.description === '' || isNaN(inputValues.value) || inputValues === '') return; // add new item in data structure

    budgetCtrl.addItem(inputValues, inputValues.type); // get new item

    var newItem = _budgetController.default.getItem(inputValues.type); // update budget on UI


    var budget = budgetCtrl.getBudget();
    UIctrl.displayBudget(budget); // clear fields

    UIctrl.clearFields(); // add markup

    UIctrl.addElement(newItem, inputValues.type); // display percentages
  ...
  function ctrlDeleteItem(elem) {
    ...
    budgetCtrl.deleteItem(elem.id); // remove morkup

    elem.parentElement.removeChild(elem); // update budget
