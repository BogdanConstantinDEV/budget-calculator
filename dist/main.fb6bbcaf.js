...
},{"./..\\images\\back.png":[["back.177f3c7d.png","images/back.png"],"images/back.png"],"_css_loader":"../node_modules/parcel-bundler/src/builtins/css-loader.js"}],"js/views/budgetViews.js":[function(require,module,exports) {
"use strict";
...
exports.addItemToHTML = addItemToHTML;
exports.displayBudget = displayBudget;
exports.displayTotals = displayTotals;
exports.deleteElement = deleteElement;
exports.changeToRed = changeToRed;
exports.markupReset = markupReset;
exports.calcPerc = calcPerc;
var DOMstrings = {
  inputType: '.add__type',
...
function addItemToHTML(newItem) {
  var markupInc = "\n        <div class=\"item clearfix\" id=\"".concat(newItem.id, "\">\n            <div class=\"item__description\">").concat(newItem.description, "</div>\n            <div class=\"right clearfix\">\n                <div class=\"item__value\">").concat(formatNumber(newItem.value, 'inc'), "</div>\n                <div class=\"item__delete\">\n                    <button class=\"item__delete--btn\"><i class=\"ion-ios-close-outline\"></i></button>\n                </div>\n            </div>\n        </div>\n    ");
  var markupExp = "\n        <div class=\"item clearfix\" id=\"".concat(newItem.id, "\">\n            <div class=\"item__description\">").concat(newItem.description, "</div>\n            <div class=\"right clearfix\">\n                <div class=\"item__value\">").concat(formatNumber(newItem.value, 'exp'), "</div>\n                <div class=\"item__percentage\">").concat(newItem.percentage, "%</div>\n                <div class=\"item__delete\">\n                    <button class=\"item__delete--btn\"><i class=\"ion-ios-close-outline\"></i></button>\n                </div>\n            </div>\n        </div>\n    ");
  document.querySelector("".concat(newItem.type === 'inc' ? DOMstrings.incList : DOMstrings.expList)).insertAdjacentHTML('beforeend', newItem.type === 'inc' ? markupInc : markupExp);
  resetInput();
} // display budget


function displayBudget(budget) {
  document.querySelector(DOMstrings.budget).textContent = "".concat(formatNumber(budget, budget > 0 ? 'inc' : 'exp'));
} // display totals


function displayTotals(totals, budget) {
  ...
  document.querySelector(DOMstrings.budget_ExpensesPercentage).textContent = budget > 0 ? "".concat(Math.round(totals.exp / totals.inc * 100), "%") : '---';
} // delete element


function deleteElement(element_id) {
  var toRemove = document.getElementById(element_id);
  toRemove.parentElement.removeChild(toRemove);
} // change input type color to red


function changeToRed() {
  var nodeList = document.querySelectorAll("".concat(DOMstrings.inputType, ",").concat(DOMstrings.inputDescription, ",").concat(DOMstrings.inputValue));
  nodeList.forEach(function (el) {
    return el.classList.toggle('red-focus');
  });
...
function markupReset() {
  ...
  document.querySelector(DOMstrings.month).textContent = new Date(Date.now()).toLocaleString('en-us', {
    month: 'long',
    year: 'numeric'
  });
} // reset input
...
function resetInput() {
  var nodeList = document.querySelectorAll("".concat(DOMstrings.inputDescription, ", ").concat(DOMstrings.inputValue));
  nodeList.forEach(function (el) {
    return el.value = '';
  });
  nodeList[0].focus();
...
function formatNumber(num, type) {
  ...
  return "".concat(type === 'inc' ? '+' : '-', " ").concat(int, ".").concat(dec, " ");
}

function calcPerc(allPerc, budget) {
  var percElem = document.querySelectorAll(DOMstrings.itemPercentage);

  if (budget > 0) {
    percElem.forEach(function (el, i) {
      el.textContent = "".concat(allPerc[i], "%");
    });
  } else {
    percElem.forEach(function (el) {
      return el.textContent = '---';
    });
  }
}
},{}],"js/models/budgetModel.js":[function(require,module,exports) {
"use strict";
...
exports.getPerc = getPerc;
exports.deleteItem = deleteItem;
exports.data = exports.Expense = exports.Income = void 0;

var _budgetViews = require("../views/budgetViews");
...
var DOMstrings = (0, _budgetViews.getDomStrings)();

var Income = function Income(type, description, value) {
  _classCallCheck(this, Income);

  this.type = type;
  this.description = description;
  this.value = value;
  this.id = "inc-".concat(Date.now());
};
...
function addItem(newItem) {
  if (document.querySelector(DOMstrings.inputDescription) === '' || document.querySelector(DOMstrings.inputValue === '')) return;
  ...
  data.allItems[newItem.type].push(newItem); // calc totals

  data.totals[newItem.type] += newItem.value; // calc budged
...
function deleteItem(itemID) {
  var type = itemID.startsWith('inc') ? 'inc' : 'exp';
  var itemIndex = data.allItems[type].findIndex(function (el) {
    return el.id === itemID;
  });
  var value = data.allItems[type][itemIndex].value;
  data.allItems[type].splice(itemIndex, 1); // calc totals

  data.totals[type] -= value; // calc budget
...
},{"../views/budgetViews":"js/views/budgetViews.js"}],"js/main.js":[function(require,module,exports) {
"use strict";
...
var budgetViews = _interopRequireWildcard(require("./views/budgetViews"));

var budgetModel = _interopRequireWildcard(require("./models/budgetModel"));
...
function setupEventListeners() {
  var DOMstrings = budgetViews.getDomStrings(); // Add Listener
  ...
  document.querySelector(DOMstrings.container).addEventListener('click', function (e) {
    var element_ID = e.target.parentElement.parentElement.parentElement.parentElement.id;
...
function ctrlAddItem() {
  ...
  var inputData = budgetViews.getInput(); // add item to budget controller

  var newItem = inputData.type === 'inc' ? new budgetModel.Income(inputData.type, inputData.description, inputData.value) : new budgetModel.Expense(inputData.type, inputData.description, inputData.value);
  budgetModel.addItem(newItem); // add the item to the UI

  budgetViews.addItemToHTML(newItem); // display the budget on the UI

  budgetViews.displayBudget(budgetModel.data.budget); // display totals on the ui

  budgetViews.displayTotals(budgetModel.data.totals, budgetModel.data.budget); // calculate percentages

  budgetModel.data.allItems.exp.forEach(function (el) {
    return el.calcPercentage();
  });
  var allPerc = budgetModel.getPerc();
  budgetViews.calcPerc(allPerc, budgetModel.data.budget);
}

function ctrlDeleteItem(element_ID) {
  // delete item from the model
  budgetModel.deleteItem(element_ID); // delete element from markup

  budgetViews.deleteElement(element_ID); // display the budget on the UI
